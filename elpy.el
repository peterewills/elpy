...
(defcustom elpy-use-jupyter nil
  "Whether to enable 'emacs-jupyter' support."
  :type 'boolean
  :group 'elpy)
  ...
  (let ((map (make-sparse-keymap)))
    ...
    (if elpy-use-jupyter
        (progn
          ...
          (define-key map (kbd "C-c C-z") 'jupyter-switch-to-associate-shell)
          (define-key map (kbd "C-c C-k") 'jupyter-repl-interrupt-kernel)
          (define-key map (kbd "C-c C-K") 'jupyter-repl-interrupt-kernel)
          (define-key map (kbd "<C-return>") 'jupyter-eval-line-or-region)
          )
...
(defvar elpy-shell-map
  (let ((map (make-sparse-keymap)))
    (if elpy-use-jupyter
        (progn
          (define-key map (kbd "f") 'jupyter-eval-defun)
          (define-key map (kbd "b") 'jupyter-eval-buffer)
          (define-key map (kbd "r") 'jupyter-eval-region)
          (define-key map (kbd "e") 'jupyter-eval-line-or-region)
          )
